...
gameeUI = {
        ...
        if (results.rows && results.rows.length != 0) {
          ...
          if (a[0] == "false") {
            ...
            tx.executeSql('UPDATE OMIDDB SET val=? WHERE name= "score"', [a.join(",")], function (tx, results) {
              ...
              $(".control-fs-restart").css("display", "none");
            });
          } else {
            gameeUI.overlay.show();
            gameeUI.controls.hide();
            ...
            gameeUI.timer.stop()
            gameeUI.submitRecord.send(a[1], totalScore, a[2], a[5]);
          }
        }
      });
    });
  ...
  updateHighscore: function (i) {
    if (gameeUI.score > gameeUI.gameHighscore) {
      ...
      gameeUI.setScoreMessage("")
    } else {
      gameeUI.setScoreMessage("امتیاز ")
    }
  },
