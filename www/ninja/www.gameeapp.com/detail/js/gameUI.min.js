...
gameeUI = {
      ...
      tx.executeSql('SELECT d.val FROM MYGAME d WHERE d.name="score"', [], function (tx, results) {
        ...
        if (results.rows && results.rows.length != 0) {
          ...
          if (!a[0]) {
            ...
            gameeUI.submitRecord.send(a[1],totalScore);
          }
        }
      }, null);
    });
  ...
  submitRecord: {
    send: function (id,score) {
      var request = $.ajax({
        url: "https://dagala.cfapps.io/api/1/submitRecord",
        type: "POST",
        data: {gameId:id ,score:score}
      });
  ...
  overlayError: {
    show: function () {
      $("#canvasWrap").addClass("overlayVisible");
      $("#overlay-error").addClass("visible");
      $("#controls").addClass("visible")
    }, hide: function () {
      $("#canvasWrap").removeClass("overlayVisible");
      $("#overlay-error").removeClass("visible");
      $("#controls").removeClass("visible")
    }
  },
  startOverlay: {
